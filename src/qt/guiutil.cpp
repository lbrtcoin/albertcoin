...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("albertcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("albertcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "albertcoin:");
    }
...
{
    QString ret = QString("albertcoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "albertcoin.desktop";
    return GetAutostartDir() / strprintf("albertcoin-%s.lnk", chain);
}
